...
var formatDate = function(date, time) {
    // We are given a time without a timezone. It is New York time.
    // If we convert to time, it will assume it is local time on the
    // machine and converting to an ISO string will add the wrong time
    // zone. Add EDT. This works now, but will be wrong when we switch
    // to EST.
    //
    // Since calendar is in same time zone as the given time, we could provide
    // a date/time without a time zone, but I don't know the format
    var d = new Date(date + ' ' + time + ' EDT');
    var s = d.toISOString()
    //console.log(s);
    return {dateTime: s};
}
...
var fetchData = function(cb) {
    var start = new Date();
    var end = new Date(start.getTime() + 1000 * 60 * 60 * 24 * 8);
    var obj = {
